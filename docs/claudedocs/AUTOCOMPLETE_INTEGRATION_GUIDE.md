...
### 인덱스 기반 O(1) 검색
**기존**: O(n×m) 선형 검색 (모든 질문을 순회하며 문자열 비교)
**최적화 후**: O(1) 단어 조회 + 인덱스 기반 검색
...
입력: "PDF 읽기"
→ 단어 분리: ["PDF", "읽기"]
→ O(1) 인덱스 조회: Set(1, 3, 7) ∩ Set(2, 5, 9) = Set()
→ 접두사 매칭 및 점수 계산
→ 상위 5개 반환
```

### 특징
- **접두사 매칭**: 단어의 일부만 입력해도 검색 (예: "PDF" → "PDF에서", "PDF는")
- **점수 기반 정렬**: 매칭 품질에 따라 자동 정렬
- **실시간 업데이트**: 질문 목록 변경 시 인덱스 자동 재구축
