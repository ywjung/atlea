...
class FeedbackAnalyzer:
    """사용자 피드백 분석 및 패턴 학습 (PostgreSQL 영구 저장 지원)"""
    ...
    def _learn_patterns(self):
        ...
        self.learned_patterns['common_negative_patterns'] = negative_patterns[:5]

        positive_patterns = self._extract_patterns(positive_feedbacks)
        self.learned_patterns['common_positive_patterns'] = positive_patterns[:5]
    ...
    def _generate_system_recommendations(self) -> List[str]:
        ...
        if positive_rate < 50:
            recommendations.append("만족도가 낮습니다. 답변 품질 개선이 필요합니다.")
        ...
        for level, counts in self.stats['feedback_by_confidence'].items():
            ...
            if total_level > 5:
                ...
                if positive_rate_level < 50:
                    recommendations.append(f"{level} 신뢰도 응답의 만족도가 낮습니다.")
        ...
        for source_group, counts in self.stats['feedback_by_sources'].items():
            ...
            if total_sources > 5:
                ...
                if positive_rate_sources < 50:
                    recommendations.append(f"출처 {source_group}일 때 만족도가 낮습니다.")
    ...
    def reset_statistics(self):
        ...
        # PG에서도 삭제
        self._delete_pg_data()
        logger.info("피드백 통계 초기화됨")

    # ===== PostgreSQL Persistence =====

    def _save_to_pg(self):
        """PostgreSQL에 피드백 데이터 저장 (SystemConfig key-value)"""
        try:
            ...
            with SyncSessionFactory() as db:
                ...
                self._upsert_config(db, "feedback:history", history_json, "json")
                ...
                self._upsert_config(db, "feedback:stats", stats_json, "json")

                # Save patterns
                patterns_json = json.dumps(self.learned_patterns, ensure_ascii=False)
                self._upsert_config(db, "feedback:patterns", patterns_json, "json")

                db.commit()

            logger.debug("피드백 데이터 PG 저장 완료")
        except Exception as e:
            logger.error(f"PG 저장 실패: {e}")

    def _upsert_config(self, db, key: str, value: str, value_type: str = "string"):
        """SystemConfig upsert helper"""
        from .database.models.system_config import SystemConfig
        ...
        if existing:
            ...
            db.add(SystemConfig(key=key, value=value, value_type=value_type))

    def _load_from_pg(self):
        """PostgreSQL에서 피드백 데이터 로드"""
        try:
            from .database.connection import SyncSessionFactory
            from .database.models.system_config import SystemConfig
            from sqlalchemy import select
            ...
            with SyncSessionFactory() as db:
                ...
                history_row = db.execute(
                    select(SystemConfig).where(SystemConfig.key == "feedback:history")
                ).scalar_one_or_none()
                if history_row:
                    self.feedback_history = json.loads(history_row.value)

                # Load stats
                stats_row = db.execute(
                    select(SystemConfig).where(SystemConfig.key == "feedback:stats")
                ).scalar_one_or_none()
                if stats_row:
                    stats = json.loads(stats_row.value)
                ...
                # Load patterns
                patterns_row = db.execute(
                    select(SystemConfig).where(SystemConfig.key == "feedback:patterns")
                ).scalar_one_or_none()
                if patterns_row:
                    self.learned_patterns = json.loads(patterns_row.value)

            if self.feedback_history:
                logger.info(f"PG에서 피드백 데이터 로드 완료: {len(self.feedback_history)}개")

        except Exception as e:
            logger.debug(f"PG 로드 실패 (초기 상태 사용): {e}")

    def _delete_pg_data(self):
        """PostgreSQL에서 피드백 데이터 삭제"""
        try:
            from .database.connection import SyncSessionFactory
            from .database.models.system_config import SystemConfig
